...
public class ShadowPearlItem extends Item {
    ...
    /*public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        user.setVelocity(velocity.add(rotationVector.x * 0.1 + (rotationVector.x * 1.5 - velocity.x), 0, rotationVector.z * 0.1 + (rotationVector.z * 1.5 - velocity.z)));

        for(int i=0; i<=500; ++i){user.setPose(EntityPose.FALL_FLYING);}
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        if (user instanceof PlayerEntity playerEntity) {
            ...
            playerEntity.addVelocity(rotationVector.x * 0.1 + (rotationVector.x * 1.5 - velocity.x), (double) -1, rotationVector.z * 0.1 + (rotationVector.z * 1.5 - velocity.z));
